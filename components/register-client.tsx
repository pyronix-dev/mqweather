...
export default function RegisterPage({ initialUser }: { initialUser: any }) {
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        try {
            try {
                // Determine endpoint based on what was provided (Email or Phone)
                const endpoint = formData.email ? '/api/auth/verify-otp-email' : '/api/auth/verify-otp-phone'

                const payload = formData.email
                    ? { email: formData.email, code: otp }
                    : { phone: formData.phone, code: otp }
                ...
                if (data.success) {
                    if (data.redirect) {
                        ...
                        // Force refresh or redirect manual
