              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4">
                {dailyData.map((day, idx) => (
                  ...
                  >
                    ...
                    {/* Temperature large */}
                    <p className="text-3xl font-bold text-slate-800 mb-1">{Math.round(day.max)}°</p>

                    <div className="text-xs text-slate-500 mb-1">
                      <span className="font-semibold text-slate-700">Max {Math.round(day.max)}°</span>
                    </div>
                    <div className="text-xs text-slate-400 mb-3">Min {Math.round(day.min)}°</div>

                    {/* Weather condition */}
                    <p className="text-xs text-slate-600 font-medium mb-4 pb-3 border-b border-slate-100">
                      {getWeatherLabel(day.precipitation)}
                    </p>
